...
int example_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __boolean_encode_array(buf, offset + pos, maxlen - pos, &this->enable, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int example_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __boolean_decode_array(buf, offset + pos, maxlen - pos, &this->enable, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    uint64_t hash = 0x1ed7555edd4b102eLL;
