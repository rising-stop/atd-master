...
class DDRenderInterfaceCoreGL final : public dd::RenderInterface {
 public:
  ...
  void drawGlyphList(const dd::DrawVertex *glyphs, int count,
    ...
    glUniform2f(textProgram_ScreenDimensions, static_cast<GLfloat>(WindowWidth),
  ...
  void setupShaderPrograms() {
    ...
    //
    {
      ...
      textProgram_GlyphTextureLocation =
          glGetUniformLocation(textProgram, "u_glyphTexture");
      if (textProgram_GlyphTextureLocation < 0) {
        errorF("Unable to get u_glyphTexture uniform location!");
      }

      textProgram_ScreenDimensions =
          glGetUniformLocation(textProgram, "u_screenDimensions");
  ...
  static void checkGLError(const char *file, const int line) {
    ...
    while ((err = glGetError()) != GL_NO_ERROR) {
      errorF("%s(%d) : GL_CORE_ERROR=0x%X - %s", file, line, err,
             errorToString(err));
    }
  }
 ...
 private:
  ...
  // In this demo, it consists of the camera's view and projection matrices
  // only.
  Matrix4 mvpMatrix;
...
const char *DDRenderInterfaceCoreGL::textVertShaderSrc =
    ...
    "1.0;\n"
    "    float y = 1.0 - ((2.0 * (in_Position.y - 0.5)) / "
...
class OpenGL_Frame : public atd::utility::Singleton {
 ...
 public:
   ...
   */
  void register_CallBack(void (*func)(const ThreadData &)) {
    ThreadData tmp_thread;
    tmp_thread.init(func, ddRenderIfaceGL_);
    draw_task_.push_back(std::move(tmp_thread));
  }
  ...
  void spin() {
    ...
    while (!glfwWindowShouldClose(ptr_mainwindow_)) {
      ...
      deltaTime.milliseconds =
          static_cast<std::int64_t>(deltaTime.seconds * 1000.0);
    }
 ...
 private:
  GLFWwindow *Init4GLFW(int windowWidth, int windowHeight) {
    ...
    GLFWwindow *window =
        glfwCreateWindow(windowWidth, windowHeight,
                         "ATD Autonomous Driving System", nullptr, nullptr);
    if (!window) {
    ...
    if (!gl3wIsSupported(3, 2)) {
      errorF(
          "This sample application requires at least OpenGL version 3.2 to "
          "run!");
      return nullptr;
    }
    ...
    ddRenderIfaceGL_ = new DDRenderInterfaceCoreGL();
  ...
  void Imgui_Setting() {
    ...
    ImGui_ImplGlfw_InitForOpenGL(ptr_mainwindow_, true);
  ...
  void single_Frame() {
    ...
    for (const auto &td : draw_task_) {
      jobQ_.pushJob([td]() { td.threadDrawFunc(td); });
    }

    ddRenderIfaceGL_->prepareDraw(camera.vpMatrix);
  ...
  JobQueue jobQ_;
  std::vector<ThreadData> draw_task_;
  DDRenderInterfaceCoreGL *ddRenderIfaceGL_;
