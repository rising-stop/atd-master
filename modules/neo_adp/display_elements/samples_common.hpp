...
static inline double getTimeSeconds() { return glfwGetTime(); }
...
static inline double getTimeSeconds() { return getTimeMilliseconds() * 0.001f; }
...
struct Keys {
  ...
  bool dDown;
  // Flags:
  bool showLabels;  // True if object labels are drawn. Toggle with the space
                    // bar.
  bool showGrid;    // True if the ground grid is drawn. Toggle with the return
  bool default_eye;
  // key.
} keys;
...
struct Camera {
  ...
  // Tunable values:
  float mouse_movementSpeed = 20.0f;
  ...
  void pitch(const float angle) {
    ...
    forward =
        rotateAroundAxis(forward, right, angle);  // Calculate new forward.
  ...
  void move(const MoveDir dir, const float amount) {
    switch (dir) {
      ...
      case Camera::Upside:
        eye += up * amount;
        break;
      case Camera::Downside:
        eye -= up * amount;
        break;
    }
  }

  void move(const float forward_amount, const float right_amount,
            const float up_amount) {
    eye += forward * forward_amount;
    eye += right * right_amount;
    eye += up * up_amount;
  }
  ...
  void checkKeyboardMovement() {
    ...
    if (keys.aDown) {
      move(Camera::Left, moveSpeed);
    }
    if (keys.dDown) {
      move(Camera::Right, moveSpeed);
    }
    if (keys.wDown) {
      move(Camera::Forward, moveSpeed);
    }
    if (keys.sDown) {
      move(Camera::Back, moveSpeed);
    }
  }

  void checkMouseMovement() {
    const float moveSpeed = mouse_movementSpeed * deltaTime.seconds;
    if (!mouse.rightButtonDown) {
      return;
    }
    float moveSpeed_x = 0.0f, moveSpeed_y = 0.0f;
    float square_root =
        sqrt(mouse.deltaX * mouse.deltaX + mouse.deltaY * mouse.deltaY);
    if (square_root < 1) {
      moveSpeed_x = 0.0f;
      moveSpeed_y = 0.0f;
    } else {
      moveSpeed_x = moveSpeed * mouse.deltaX / square_root;
      moveSpeed_y = moveSpeed * mouse.deltaY / square_root;
    }

    move(0.0f, moveSpeed_x, moveSpeed_y);
  }
  ...
  void checkKey1() {
    ...
    keys.default_eye = false;
  }

  void updateMatrices() {
    viewMatrix = Matrix4::lookAt(Point3(eye), getTarget(), up);
    vpMatrix = projMatrix * viewMatrix;  // Vectormath lib uses column-major OGL
                                         // style, so multiply P*V*M
  ...
  Point3 getTarget() const {
    return Point3(eye[0] + forward[0], eye[1] + forward[1],
                  eye[2] + forward[2]);
  }

  bool isPointInsideFrustum(const float x, const float y, const float z) const {
    for (int i = 0; i < 6; ++i) {
      if ((planes[i][A] * x + planes[i][B] * y + planes[i][C] * z +
           planes[i][D]) <= 0.0f) {
        return false;
      }
    }
  ...
  static Vector3 rotateAroundAxis(const Vector3& vec, const Vector3& axis,
                                  const float angle) {
    const float sinAng = std::sin(angle);
    const float cosAng = std::cos(angle);
...
static void mousePositionCallback(GLFWwindow* window, const double xPos,
                                  const double yPos) {
  (void)window;  // Unused.
  ...
  if (mouse.deltaX > Mouse::MaxDelta) {
    mouse.deltaX = Mouse::MaxDelta;
  } else if (mouse.deltaX < -Mouse::MaxDelta) {
    mouse.deltaX = -Mouse::MaxDelta;
  }
  if (mouse.deltaY > Mouse::MaxDelta) {
    mouse.deltaY = Mouse::MaxDelta;
  } else if (mouse.deltaY < -Mouse::MaxDelta) {
    mouse.deltaY = -Mouse::MaxDelta;
  }
}
...
static void mouseButtonCallback(GLFWwindow* window, const int button,
  ...
  if (button == GLFW_MOUSE_BUTTON_LEFT) {
    ...
    mouse.middleButtonDown = (action != GLFW_RELEASE);
  }
}

static void keyCallback(GLFWwindow* window, const int key, const int scancode,
                        const int action, const int mods) {
  // Unused.
  (void)window;
  (void)scancode;
...
struct MainThreadChecker {
  ...
  MainThreadChecker() : mainThreadId(std::this_thread::get_id()) {}

  bool operator()() const { return std::this_thread::get_id() == mainThreadId; }
} isMainThread;
