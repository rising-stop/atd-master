...
void ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  ScrollbarEx(bb, id, axis, &window->Scroll[axis], size_avail, size_contents,
              rounding_corners);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  ImRect bb = bb_frame;
  bb.Expand(
      ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
  ...
  const float grab_h_pixels =
      ImClamp(scrollbar_size_v * (size_avail_v / win_size_v), style.GrabMinSize,
              scrollbar_size_v);
  ...
  if (held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    const float scroll_v_norm =
        ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter -
                    grab_h_norm * 0.5f) /
                   (1.0f - grab_h_norm));
    *p_scroll_v = IM_ROUND(scroll_v_norm *
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size,
  ...
  return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding, bg_col,
                       tint_col);
}
...
void ImGui::Bullet() {
  ...
  const float line_height = ImMax(
      ImMin(window->DC.CurrLineSize.y, g.FontSize + g.Style.FramePadding.y * 2),
      g.FontSize);
  const ImRect bb(window->DC.CursorPos,
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
  ...
  if (flags & ImGuiSeparatorFlags_Vertical) {
    // Vertical separator, for menu bars (use current line height). Not exposed
    // because it is misleading and it doesn't have an effect on regular layout.
    ...
    ImGuiColumns* columns = (flags & ImGuiSeparatorFlags_SpanAllColumns)
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
  ...
  // Round width and redistribute remainder left-to-right (could make it an
  // option of the function?) Ensure that e.g. the right-most tab of a shrunk
  // tab-bar always reaches exactly at the same distance from the right-most
  // edge of the tab bar separator.
  width_excess = 0.0f;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value,
  ...
  if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    RenderTextClipped(frame_bb.Min + style.FramePadding,
                      ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL,
  ...
  if (ImGuiWindow* popup_window = FindWindowByName(name))
    if (popup_window->WasActive) {
      ...
      ImVec2 pos = FindBestWindowPosForPopupEx(
          frame_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection,
...
bool ImGui::DataTypeApplyOpFromText(const char* buf,
  ...
  if (data_type == ImGuiDataType_S32) {
    ...
    // We don't bother handling support for legacy operators since they are a
    // little too crappy. Instead we will later implement a proper expression
    // evaluator in the future.
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data,
  ...
  switch (data_type) {
    case ImGuiDataType_S8:
      return ClampBehaviorT<ImS8>((ImS8*)p_data, *(const ImS8*)p_min,
                                  *(const ImS8*)p_max);
    case ImGuiDataType_U8:
      return ClampBehaviorT<ImU8>((ImU8*)p_data, *(const ImU8*)p_min,
                                  *(const ImU8*)p_max);
    case ImGuiDataType_S16:
      return ClampBehaviorT<ImS16>((ImS16*)p_data, *(const ImS16*)p_min,
                                   *(const ImS16*)p_max);
    case ImGuiDataType_U16:
      return ClampBehaviorT<ImU16>((ImU16*)p_data, *(const ImU16*)p_min,
                                   *(const ImU16*)p_max);
    case ImGuiDataType_S32:
      return ClampBehaviorT<ImS32>((ImS32*)p_data, *(const ImS32*)p_min,
                                   *(const ImS32*)p_max);
    case ImGuiDataType_U32:
      return ClampBehaviorT<ImU32>((ImU32*)p_data, *(const ImU32*)p_min,
                                   *(const ImU32*)p_max);
    case ImGuiDataType_S64:
      return ClampBehaviorT<ImS64>((ImS64*)p_data, *(const ImS64*)p_min,
                                   *(const ImS64*)p_max);
    case ImGuiDataType_U64:
      return ClampBehaviorT<ImU64>((ImU64*)p_data, *(const ImU64*)p_min,
                                   *(const ImU64*)p_max);
    case ImGuiDataType_Float:
      return ClampBehaviorT<float>((float*)p_data, *(const float*)p_min,
                                   *(const float*)p_max);
    case ImGuiDataType_Double:
      return ClampBehaviorT<double>((double*)p_data, *(const double*)p_min,
                                    *(const double*)p_max);
    case ImGuiDataType_COUNT:
      break;
  }
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
  ...
  const bool is_power =
      (power != 1.0f && is_decimal && is_clamped && (v_max - v_min < FLT_MAX));
  ...
  if (is_power) {
    ...
    v_cur =
        v_min + (SIGNEDTYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) *
                    (v_max - v_min);
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v,
  ...
  if (g.CurrentWindow->DC.ItemFlags & ImGuiItemFlags_ReadOnly) return false;
...
// SliderFloat(), SliderInt() etc. or examples in 'Demo->Widgets->Data Types' to
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
  ...
  if (power != 1.0f)
    IM_ASSERT(p_min != NULL &&
              p_max != NULL);  // When using a power curve the drag needs to
                               // have known bounds
  ...
  // Our current specs do NOT clamp when using CTRL+Click manual input, but we
  // should eventually add a flag for that..
  if (temp_input_is_active)
  ...
  const bool value_changed =
      DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format, power,
                   ImGuiDragFlags_None);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type,
  ...
  for (int i = 0; i < components; i++) {
    ...
    value_changed |=
        DragScalar("", data_type, p_data, v_speed, p_min, p_max, format, power);
    PopID();
    PopItemWidth();
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min,
  ...
  bool value_changed = DragScalar("##min", ImGuiDataType_Float, v_current_min,
                                  v_speed, &min, &max, format, power);
  PopItemWidth();
  SameLine(0, g.Style.ItemInnerSpacing.x);
  ...
  value_changed |=
      DragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed, &min,
                 &max, format_max ? format_max : format, power);
  PopItemWidth();
  SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min,
  ...
  bool value_changed =
      DragInt("##min", v_current_min, v_speed, min, max, format);
  PopItemWidth();
  SameLine(0, g.Style.ItemInnerSpacing.x);
  ...
  value_changed |= DragInt("##max", v_current_max, v_speed, min, max,
...
float ImGui::SliderCalcRatioFromValueT(ImGuiDataType data_type, TYPE v,
                                       TYPE v_min, TYPE v_max, float power,
                                       float linear_zero_pos) {
  if (v_min == v_max) return 0.0f;

  const bool is_power = (power != 1.0f) && (data_type == ImGuiDataType_Float ||
  ...
  if (is_power) {
    if (v_clamped < 0.0f) {
      const float f =
          1.0f - (float)((v_clamped - v_min) / (ImMin((TYPE)0, v_max) - v_min));
...
// FIXME: Move some of the code into SliderBehavior(). Current responsibility is
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id,
                            ...
                            const TYPE v_max, const char* format, float power,
  ...
  // For power curve sliders that cross over sign boundary we want the curve to
  // be symmetric around 0.0f
  ...
  if (is_power && v_min * v_max < 0.0f) {
    ...
    const FLOATTYPE linear_dist_max_to_0 =
        ImPow(v_max >= 0 ? (FLOATTYPE)v_max : -(FLOATTYPE)v_max,
              (FLOATTYPE)1.0f / power);
    linear_zero_pos = (float)(linear_dist_min_to_0 /
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        if ((decimal_precision > 0) || is_power) {
          ...
          if ((v_range >= -100.0f && v_range <= 100.0f) ||
            ...
            delta =
                ((delta < 0.0f) ? -1.0f : +1.0f) /
                (float)
                    v_range;  // Gamepad/keyboard tweak speeds in integer steps
          else
            delta /= 100.0f;
        }
    ...
    if (set_new_value) {
      ...
      if (is_power) {
        ...
        if (is_decimal) {
          ...
          // box so we round above This code is carefully tuned to work with
          // large values (e.g. high ranges of U64) while preserving this
          // property..
      ...
      // Round to user desired precision based on format string
      v_new =
          RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format, data_type, v_new);
...
// Note that Drag/Slider functions are currently NOT forwarding the min/max
// values clamping values! This is intended: this way we allow CTRL+Click manual
// input to set a value out of bounds, for maximum flexibility. However this may
// not be ideal for all uses, as some user code may break on out of bound
// values. In the future we should add flags to Slider/Drag to specify how to
// enforce min/max values with CTRL+Click. See GitHub issues #1829 and #3209 In
// the meanwhile, you can easily "wrap" those functions to enforce clamping,
// using wrapper functions, e.g.
//   bool SliderFloatClamp(const char* label, float* v, float v_min, float
//   v_max)
//   {
//      float v_backup = *v;
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
  ...
  if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags)) {
    ...
    DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data,
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast,
                       int decimal_precision, ImGuiInputTextFlags flags) {
  char format[16] = "%f";
...
static bool InputTextFilterCharacter(unsigned int* p_char,
  ...
  // Filter private Unicode range. GLFW on OSX seems to send private characters
  // for special keys like arrow keys (FIXME)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
  ...
  if (is_multiline) {
    ...
    bool child_visible = BeginChildEx(
        label, id, frame_bb.GetSize(), true,
  ...
  if (init_state && g.ActiveId != id) {
    ...
    state->CurLenA =
        (int)(buf_end - buf);  // We can't get the result from ImStrncpy() above
                               // because it is not UTF-8 aware. Here we'll cut
                               // off malformed UTF-8.
    ...
    // TextAIsValid && TextA == buf ? (untested) and discard undo stack if user
    // buffer has changed.
  ...
  if (g.ActiveId != id && init_make_active) {
    ...
    if (is_multiline)
      g.ActiveIdUsingKeyInputMask |=
          ...
          ((ImU64)1 << ImGuiKey_PageDown);  // FIXME-NAV: Page up/down actually
                                            // not supported yet by widget, but
                                            // claim them ahead.
    if (flags &
        (ImGuiInputTextFlags_CallbackCompletion |
         ImGuiInputTextFlags_AllowTabInput))  // Disable keyboard tabbing out as
  ...
  if (g.ActiveId == id) {
    ...
    // It is ill-defined whether the back-end needs to send a \t character when
    // pressing the TAB keys. Win32 and GLFW naturally do it but not SDL.
    const bool ignore_char_inputs =
        (io.KeyCtrl && !io.KeyAlt) || (is_osx && io.KeySuper);
    if ((flags & ImGuiInputTextFlags_AllowTabInput) &&
        IsKeyPressedMap(ImGuiKey_Tab) && !ignore_char_inputs && !io.KeyShift &&
        !is_readonly)
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    IM_ASSERT(io.KeyMods == GetMergedKeyModFlags() &&
              ...
              "io.KeyMods");  // We rarely do this check, but if anything let's
                              // do it here.
    ...
    const bool is_osx_shift_shortcut =
        is_osx &&
        (io.KeyMods == (ImGuiKeyModFlags_Super | ImGuiKeyModFlags_Shift));
    ...
    const bool is_paste =
        ...
        !is_readonly;
    const bool is_undo = ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Z)) &&
  ...
  if (g.ActiveId == id) {
    ...
    if (cancel_edit) {
      ...
      // value changes the current buffer contents.
      if (!is_readonly && strcmp(buf, state->InitialTextA.Data) != 0) {
        // Push records into the undo stack so we can CTRL+Z the revert
        // operation itself
    ...
    bool apply_edit_back_to_user_buffer =
        !cancel_edit ||
        (enter_pressed && (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
  ...
  // does a final application of the value)
    ...
    if (render_cursor && state->CursorFollow) {
      ...
      if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
        ...
        if (cursor_offset.x < state->ScrollX)
          state->ScrollX =
              IM_FLOOR(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
        else if (cursor_offset.x - inner_size.x >= state->ScrollX)
          state->ScrollX =
              IM_FLOOR(cursor_offset.x - inner_size.x + scroll_increment_x);
      } else {
        state->ScrollX = 0.0f;
      }
    ...
    if (render_selection) {
      ...
      for (const ImWchar* p = text_selected_begin; p < text_selected_end;) {
        ...
        if (rect_pos.y < clip_rect.y) {
          ...
          if (rect_size.x <= 0.0f)
            rect_size.x =
                IM_FLOOR(g.Font->GetCharAdvance((ImWchar)' ') *
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  if ((flags & ImGuiColorEditFlags_InputHSV) &&
    ...
    // Hue is lost when converting from greyscale rgb (saturation=0). Restore
    // it.
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB |
    ...
    const float w_item_last = ImMax(
        1.0f, IM_FLOOR(w_inputs - (w_item_one + style.ItemInnerSpacing.x) *
                                      (components - 1)));
    ...
    const int fmt_idx =
        hide_prefix ? 0 : (flags & ImGuiColorEditFlags_DisplayHSV) ? 2 : 1;
  ...
  if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
    const float text_offset_x = (flags & ImGuiColorEditFlags_NoInputs)
                                    ? w_button
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  if ((flags & ImGuiColorEditFlags__DisplayMask) == 0)
    flags |=
        ImGuiColorEditFlags__OptionsDefault & ImGuiColorEditFlags__DisplayMask;
  if ((flags & ImGuiColorEditFlags__DataTypeMask) == 0)
    flags |=
        ImGuiColorEditFlags__OptionsDefault & ImGuiColorEditFlags__DataTypeMask;
  if ((flags & ImGuiColorEditFlags__PickerMask) == 0)
    flags |=
        ImGuiColorEditFlags__OptionsDefault & ImGuiColorEditFlags__PickerMask;
  if ((flags & ImGuiColorEditFlags__InputMask) == 0)
    flags |=
        ImGuiColorEditFlags__OptionsDefault & ImGuiColorEditFlags__InputMask;
  IM_ASSERT(ImIsPowerOfTwo(
      flags &
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // _OpenOnDoubleClick=1 and _OpenOnArrow=0) This makes _OpenOnArrow have a
  // subtle effect on _OpenOnDoubleClick: arrow click reacts on Down rather than
  // Up. It is rather standard that arrow click react on Down rather than Up and
  // we'd be tempted to make it the default (by removing the _OpenOnArrow test
  // below), however this would have a perhaps surprising effect on
  // CollapsingHeader()? So right now we are making this optional. May evolve
  // later. We set ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick
  // because we want the item to be active on the initial MouseDown in order for
  ...
  if (!is_leaf) {
    ...
    if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir == ImGuiDir_Left &&
        is_open) {
      toggled = true;
...
bool ImGui::Selectable(const char* label, bool selected,
  ...
  const float min_x = (flags & ImGuiSelectableFlags_SpanAllColumns)
                          ? window->ContentRegionRect.Min.x
                          : pos.x;
  const float max_x = (flags & ImGuiSelectableFlags_SpanAllColumns)
                          ? window->ContentRegionRect.Max.x
                          : GetContentRegionMaxAbs().x;
  ...
  // if (g.IO.KeyCtrl) { GetForegroundDrawList()->AddRect(bb_align.Min,
  // bb_align.Max, IM_COL32(255, 0, 0, 255)); } if (g.IO.KeyCtrl) {
  // GetForegroundDrawList()->AddRect(bb_enlarged.Min, bb_enlarged.Max,
  ...
  RenderTextClipped(text_min, text_max, label, NULL, &label_size,
                    style.SelectableTextAlign, &bb_enlarged);
...
bool ImGui::ListBoxHeader(const char* label, int items_count,
  ...
  // add this extra bit if items_count <= height_in_items. It is slightly dodgy,
...
void ImGui::ListBoxFooter() {
  ...
  // size in LastItemRect) We call SameLine() to restore DC.CurrentLine* data
  SameLine();
  ...
  if (values_count >= values_count_min) {
    ...
    if (hovered && inner_bb.Contains(g.IO.MousePos)) {
      ...
      for (int index = 0; index < data_set_num; index++) {
        ...
        if (index != (data_set_num - 1)) {
          tip_info.append("\n");
        }
      }

      SetTooltip(tip_info.c_str());
      idx_hovered = v_idx;
    }
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      // scoring in advance for multiple window (probably not worth the
      // hassle/cost)
  ...
  window->DC.MenuBarOffset.x =
      window->DC.CursorPos.x -
      window->MenuBarRect()
          .Min.x;  // Save horizontal position so next append can reuse it. This
...
bool ImGui::BeginMainMenuBar() {
  ...
  SetNextWindowSize(
      ImVec2(g.IO.DisplaySize.x, g.NextWindowData.MenuBarOffsetMinVal.y +
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  // ever becoming a problem we can switch to use e.g. a ImGuiStorager mapping
  ...
  if (menuset_is_open)
    g.NavWindow =
        window;  // Odd hack to allow hovering across menus of a same menu-set
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    RenderArrow(window->DrawList,
                pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w +
                                 g.FontSize * 0.30f,
                             0.0f),
  ...
  if (menu_is_open) {
    ...
    menu_is_open = BeginPopupEx(
        id, flags);  // menu_is_open can be 'false' when the popup is completely
...
void ImGui::EndMenu() {
  ...
  // EndMenuBar() wants the catch the last Left<>Right inputs. However, it means
  // that with the current code, a BeginMenu() from outside another menu or a
  // menu-bar won't be closable with the Left direction.
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected,
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    // this situation we render neither the shortcut neither the selected tick
    // mark
    float w = label_size.x;
    window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
    PushStyleVar(ImGuiStyleVar_ItemSpacing,
    ...
    float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
    pressed =
        Selectable(label, false, flags | ImGuiSelectableFlags_SpanAvailWidth,
                   ImVec2(min_w, 0.0f));
    if (shortcut_w > 0.0f) {
      PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
      RenderText(pos + ImVec2(window->DC.MenuColumns.Pos[1] + extra_w, 0.0f),
                 shortcut, NULL, false);
      PopStyleColor();
    }
    if (selected)
...
ImGuiTabBar::ImGuiTabBar() {
  ...
  ScrollingAnim = ScrollingTarget = ScrollingTargetDistToVisibility =
      ScrollingSpeed = 0.0f;
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb,
  ...
  // enforce the last visible order. Otherwise, the most recently inserted tabs
  // would move at the end of visible list which can be a little too confusing
  // or magic for the user.
  ...
  const float y = tab_bar->BarRect.Max.y - 1.0f;
  {
    ...
    window->DrawList->AddLine(ImVec2(separator_min_x, y),
                              ImVec2(separator_max_x, y), col, 1.0f);
  }
  return true;
}
...
void ImGui::EndTabBar() {
  ...
  if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 ||
      tab_bar_appearing)
    tab_bar->LastTabContentHeight =
        ImMax(window->DC.CursorPos.y - tab_bar->BarRect.Max.y, 0.0f);
  else
    window->DC.CursorPos.y =
        tab_bar->BarRect.Max.y + tab_bar->LastTabContentHeight;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
  ...
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ...
    width_total_contents +=
        (tab_n > 0 ? g.Style.ItemInnerSpacing.x : 0.0f) + tab->ContentWidth;
  ...
  const float width_avail =
      ImMax(tab_bar->BarRect.GetWidth() - initial_offset_x, 0.0f);
  ...
  if (width_excess > 0.0f &&
    ...
    ShrinkWidths(g.ShrinkWidthBuffer.Data, g.ShrinkWidthBuffer.Size,
                 width_excess);
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
  ...
  tab_bar->OffsetNextTab =
      offset_x;  // This is used by non-reorderable tab bar where the submission
  ...
  tab_bar->OffsetMaxIdeal =
      ImMax(offset_x_ideal - g.Style.ItemInnerSpacing.x, 0.0f);
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab) {
  ...
  if (tab_bar->ScrollingTarget > tab_x1 ||
      (tab_x2 - tab_x1 >= tab_bar->BarRect.GetWidth())) {
...
void ImGui::TabBarQueueChangeTabOrder(ImGuiTabBar* tab_bar,
                                      const ImGuiTabItem* tab, int dir) {
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar) {
  ...
  window->DC.CursorPos = ImVec2(
      tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y);
      ...
      tab_to_select =
          &tab_bar
               ->Tabs[(target_order >= 0 && target_order < tab_bar->Tabs.Size)
                          ? target_order
                          : selected_order];  // If we are at the end of the
                                              // list, still scroll to make our
                                              // tab visible
    }
...
bool ImGui::BeginTabItem(const char* label, bool* p_open,
  ...
  ImGuiTabBar* tab_bar = g.CurrentTabBar;
  if (tab_bar == NULL) {
    IM_ASSERT_USER_ERROR(tab_bar,
                         "BeginTabItem() Needs to be called between "
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
  ...
  // We make a dummy call to ItemAdd() so that attempts to use a contextual
  ...
  window->DC.CursorPos =
      tab_bar->BarRect.Min +
      ImVec2(IM_FLOOR(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
  ImVec2 pos = window->DC.CursorPos;
  ImRect bb(pos, pos + size);
  ...
  // Allow the close button to overlap unless we are dragging (in which case we
  // don't want any overlapping tabs to be hovered)
  ...
  // systems messes up with HoveredIdTimer) We test IsItemHovered() to discard
  // e.g. when another item is active or drag and drop over the tab bar (which
  // g.HoveredId ignores)
...
// BeginTabItem(). Tabs closed by the close button will automatically be flagged
// to avoid this issue.
// FIXME: We should aim to support calling SetTabItemClosed() after the tab
// submission (for next frame)
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
                                       ImGuiTabItemFlags flags,
  ...
  if (close_button_id != 0)
    if (is_contents_visible ||
        bb.GetWidth() >= g.Style.TabMinWidthForUnselectedCloseButton)
  ...
  if (close_button_visible) {
    ...
    if (CloseButton(close_button_id,
                    ImVec2(bb.Max.x - frame_padding.x * 2.0f - close_button_sz,
                           bb.Min.y)))
...
static float GetColumnWidthEx(ImGuiColumns* columns, int column_index,
